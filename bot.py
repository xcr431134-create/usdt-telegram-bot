...
def run_bot():
    ...
    # تنظيف أي webhook قديم مباشرة
    try:
        bot.delete_webhook()
        time.sleep(2)
        print("✅ تم تنظيف الـ webhook القديم")
    except Exception as e:
        print(f"ℹ️  لا يوجد webhook قديم أو خطأ: {e}")
    
    # تهيئة قاعدة البيانات
    if not init_database():
        print("⚠️  تم المتابعة بدون قاعدة البيانات")
    
    # تشغيل البوت مع أفضل إعدادات
    while True:
        try:
            print("🔄 بدء Polling مع skip_pending...")
            bot.polling(
                none_stop=True,
                timeout=60,
