...
def create_default_user(user_id):
    ...
    user_data = {
        'user_id': str(user_id),
        'username': "", 'first_name': "", 'balance': 0.75, 'referrals_count': 0, 'referrals_new': 0,
        'games_played_today': 0, 'total_games_played': 0, 'total_earned': 0.75, 'total_deposits': 0.0,
        'vip_level': 0, 'registration_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'last_activity': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'last_reset_date': datetime.now().strftime('%Y-%m-%d'),
        'withdrawal_address': "", 'registration_days': 0, 'last_daily_check': datetime.now().strftime('%Y-%m-%d')
    }
    save_user(user_data)
    return user_data
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                ON CONFLICT (user_id) DO UPDATE SET
                    username=EXCLUDED.username, first_name=EXCLUDED.first_name, balance=EXCLUDED.balance,
                    referrals_count=EXCLUDED.referrals_count, referrals_new=EXCLUDED.referrals_new,
                    games_played_today=EXCLUDED.games_played_today, total_games_played=EXCLUDED.total_games_played,
                    total_earned=EXCLUDED.total_earned, total_deposits=EXCLUDED.total_deposits,
                    vip_level=EXCLUDED.vip_level, last_activity=EXCLUDED.last_activity,
                    last_reset_date=EXCLUDED.last_reset_date, withdrawal_address=EXCLUDED.withdrawal_address,
                    registration_days=EXCLUDED.registration_days, last_daily_check=EXCLUDED.last_daily_check
            """, (
                user_data['user_id'], user_data['username'], user_data['first_name'], user_data['balance'],
                user_data['referrals_count'], user_data['referrals_new'], user_data['games_played_today'],
                user_data['total_games_played'], user_data['total_earned'], user_data['total_deposits'],
                user_data['vip_level'], user_data['registration_date'], user_data['last_activity'],
                user_data['last_reset_date'], user_data['withdrawal_address'], user_data['registration_days'],
                user_data['last_daily_check']
            ))
            conn.commit()
            return True
...
def update_user_activity(user_id):
    ...
    user['registration_days'] = (datetime.now() - registration_date).days
    save_user(user)

def get_vip_level_name(level):
    vip_names = {0: "🟢 مبتدئ", 1: "🟢 برونز", 2: "🔵 سيلفر", 3: "🟡 جولد"}
...
👤 المستخدم: {message.from_user.first_name or "مستخدم"}
...
def handle_vip_purchase(call):
    try:
        ...
        vip_names = {'bronze': '🟢 برونز VIP', 'silver': '🔵 سيلفر VIP', 'gold': '🟡 جولد VIP'}
...
📅 الوقت: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
        ...
        bot.answer_callback_query(call.id, f"✅ تم إرسال طلب شراء {vip_name} للإدارة", show_alert=True)
    except Exception as e:
...
def process_withdrawal_address(message, user):
    try:
        ...
        if len(address) < 10:
            msg = bot.send_message(message.chat.id, "❌ عنوان غير صحيح! الرجاء إرسال عنوان محفظة USDT (TRC20) صحيح:")
            bot.register_next_step_handler(msg, process_withdrawal_address, user)
            return
...
def process_withdrawal(call):
    try:
        ...
        if withdraw_type == '150': amount = 150.0
        elif withdraw_type == '300': amount = 300.0
        elif withdraw_type == '500': amount = 500.0
...
📅 الوقت: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
        ...
        bot.answer_callback_query(call.id, f"✅ تم إرسال طلب سحب {amount:.1f} USDT للإدارة", show_alert=True)
    except Exception as e:
...
📤 شارك الرابط مع أصدقائك واكسب المزيد!"""
        ...
        bot.edit_message_text(referral_text, call.message.chat.id, call.message.message_id, reply_markup=keyboard, parse_mode='Markdown')
    except Exception as e:
...
👤 المستخدم: {call.from_user.first_name or "مستخدم"}
...
📅 تاريخ التسجيل: {user['registration_date'].split()[0]}"""
        ...
        bot.edit_message_text(profile_text, call.message.chat.id, call.message.message_id, reply_markup=keyboard)
    except Exception as e:
